...
//=======================================================
let _buffer_pos         = 0;
let _buffer_positions   = null;  // вершины
let _buffer_colors      = null;  // цвета
let _buffer_uv          = null;  // координаты текстуры
let _buffer_index       = null;  // индексы костей
let _buffer_weights     = null;  // веса костей
let _buffer_pos2 = 0;


// создаем буффер
function _create_buffer(count){
    _buffer_pos = 0;
    count = count * 3; // в каждом точке 3 координаты) - считаем сколько всего цифр  
    _buffer_positions = new Float32Array( count );
    _buffer_colors    = new Float32Array( count );
}

// создаем буффер
function _create_buffer2(count){
    _buffer_pos = 0;
    _buffer_pos2 = 0;
    _buffer_positions = new Float32Array( count * 3 );
    _buffer_uv    = new Float32Array( count * 2);
}

function _create_s_buffers(count){
    _buffer_pos = 0;
    _buffer_pos2 = 0;
    _buffer_positions = new Float32Array( count*3 );
    _buffer_colors    = new Float32Array( count*3 );
    _buffer_index     = new Float32Array( count*4 );
    _buffer_weights   = new Float32Array( count*4 );
}

function _set_buff(p,c){
   _buffer_positions[_buffer_pos  ] = p.x;  
   _buffer_colors   [_buffer_pos  ] = c.r;   
   _buffer_positions[_buffer_pos+1] = p.y;  
   _buffer_colors   [_buffer_pos+1] = c.g;   
   _buffer_positions[_buffer_pos+2] = p.z;  
   _buffer_colors   [_buffer_pos+2] = c.b;   
   _buffer_pos = _buffer_pos+3;
}

function _set_buff2(p,u){
   _buffer_positions[_buffer_pos  ] = p.x;  
   _buffer_positions[_buffer_pos+1] = p.y;  
   _buffer_positions[_buffer_pos+2] = p.z;  
   _buffer_uv[_buffer_pos2  ] = u.x;  
   _buffer_uv[_buffer_pos2+1] = u.y;  
   _buffer_pos = _buffer_pos+3;
   _buffer_pos2 = _buffer_pos2+2;
}

function _set_tri(p1,p2,p3,c1,c2,c3){
    _set_buff(p1,c1);
    _set_buff(p2,c2);
    _set_buff(p3,c3);
}

function _set_s_buff(vector,color,bone_index,weights){
    _buffer_positions[_buffer_pos+0] = vector.x;
    _buffer_positions[_buffer_pos+1] = vector.y;
    _buffer_positions[_buffer_pos+2] = vector.z;
    _buffer_colors[_buffer_pos+0] = color.r;
    _buffer_colors[_buffer_pos+1] = color.g;
    _buffer_colors[_buffer_pos+2] = color.b;
    _buffer_index[_buffer_pos2+0] = bone_index.b1;
    _buffer_index[_buffer_pos2+1] = bone_index.b2;
    _buffer_index[_buffer_pos2+2] = bone_index.b3;
    _buffer_index[_buffer_pos2+3] = bone_index.b4;
    _buffer_weights[_buffer_pos2+0] = weights.w1;
    _buffer_weights[_buffer_pos2+1] = weights.w2;
    _buffer_weights[_buffer_pos2+2] = weights.w3;
    _buffer_weights[_buffer_pos2+3] = weights.w4;

    _buffer_pos = _buffer_pos+3;
    _buffer_pos2 = _buffer_pos2+4;
}
//
function _set_s_tri(p1,p2,p3,c1,c2,c3,bi1,bi2,bi3,w1,w2,w3){
    _set_s_buff(p1,c1,bi1,w1);
    _set_s_buff(p2,c2,bi2,w2);
    _set_s_buff(p3,c3,bi3,w3);
}
